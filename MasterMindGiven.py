...
def draw_board(root):
    ...
    hint_button = Button(canvas, text = "Hint homow", command = hint)
    hint_button.configure(width = 10, relief = FLAT)
    canvas.create_window(100, MAX_NUMBER_OF_MOVES*40+130, anchor = NW, window = hint_button)

    #secret button
    site_button = Button(canvas, text = "Secret", command = site)
    site_button.configure(width = 10, relief = FLAT)
    canvas.create_window(140, MAX_NUMBER_OF_MOVES*40+90, anchor = NW, window = site_button)
    # draw the canvas
    canvas.pack()

def site():
    webbrowser.open('https://nl.pornhub.com/view_video.php?viewkey=ph5c72614e015c9&t=660')
...
def hint():
    ...
    plaats = getal + 1
    kleur = random_combination[getal]
    messagebox.showinfo("Hint", "plaats: "+str(plaats)+"\nkleur: "+str(kleur) + "\n"+str(random_combination))
